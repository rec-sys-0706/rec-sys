...
def parse_behaviors(src_path, dest_path):
    ...
    behaviors['history'] = behaviors['history'].fillna('') # Handle missing values.
    ...
    behaviors['history'] = behaviors['history'].apply(lambda h: ' '.join(set(h.split()))) # Remove duplicated values in 'history'.
    ...
    behaviors = behaviors.rename(columns={'history': 'clicked_news'})
    behaviors.to_csv(dest_path,
                     sep='\t',
                     index=False,
                     columns=['user_id', 'clicked_news', 'candidate_news', 'clicked'])

def parse_news(src_path, dest_dir):
    """
    Returns:
        category2int
        word2int
    ...
    category2int = {category: idx for idx, category in enumerate(categories)}
    # ! processing words
    tokens = []
...
if __name__ == '__main__':
    ...
    parse_news('data/MINDsmall_train/news.tsv', 'data/MINDsmall_train')